...
def messages_view(request):
    ...
    if request.user.is_authenticated:
        ...
        posts = []
        temp = []
        morecounter = request.session.get('secondcounter',2)
        for i in models.Post.objects.all().order_by('-timestamp'):
            temp.append(i)
        posts = temp[:morecounter]
        # TODO Objective 10: check if user has like post, attach as a new attribute to each post
        
        for p in posts:
            checklike = p.likes.all().filter(user=user_info)
            if len(checklike) == 1:
                p.bool = True

            countlike = p.likes.all()
            p.likecount = len(countlike)
        ...
        if request.method == 'POST':
            ...
            try:
                if form.is_valid():
                    print(request.POST)
                    user = form.save()
                    update_session_auth_hash(user_info)
                    return redirect('login:login_view')
            except:
                return redirect('login:login_view')
        else:
...
def accountupdate_view(request):
    ...
    if request.method == 'POST':
        ...
        if birthday != '':
            ...
            user_info.save()
        if interests != '':
            interest = models.Interest.objects.filter(label=interests)
            boolcheckinterest = interest.exists()
            if not boolcheckinterest:
                user_info.interests.add(models.Interest.objects.create(label=interests))
                user_info.save()
            else:
...
def people_view(request):
    ...
    if request.user.is_authenticated:
        ...
        morecount = request.session.get('counter',2)
        all_people = []
        people = []
        for u in models.UserInfo.objects.all().exclude(user=request.user):
            if u not in user_info.friends.all():
                people.append(u)
        all_people = people[:morecount]    
        
        # TODO Objective 5: create a list of all friend requests to current user
        friend_requests = []
        for f in models.FriendRequest.objects.all().filter(to_user=user_info):
                friend_requests.append(f)
        for a in all_people:
            if models.FriendRequest.objects.all().filter(to_user=a, from_user= user_info): #from objective 10
                a.bool = True
        context = { 'user_info' : user_info,
...
def like_view(request):
    ...
    if postIDReq is not None:
        ...
        if request.user.is_authenticated:
            ...
            user_info = models.UserInfo.objects.get(user=request.user)
            models.Post.objects.get(id=postID).likes.add(user_info)
...
def more_post_view(request):
    ...
    if request.user.is_authenticated:
        ...
        sessionvar = request.session.get('secondcounter',2)
        request.session['secondcounter'] = sessionvar + 1
        # return status='success'
        return HttpResponse()
...
def more_ppl_view(request):
    ...
    if request.user.is_authenticated:
        ...
        sessionvar = request.session.get('counter',2) #2 because the counter on top starts from 2
        request.session['counter'] = sessionvar + 1
        # return status='success'
        return HttpResponse()
...
def friend_request_view(request):
    ...
    if frID is not None:
        ...
        if request.user.is_authenticated:
            ...
            user_receiver = models.UserInfo.objects.get(user=request.user)
            if not models.FriendRequest.objects.filter(to_user=user_info, from_user=user_receiver).exists():
                models.FriendRequest.objects.create(to_user=user_info, from_user=user_receiver)
            else:
                print("already sent")
            # return status='success'
            return HttpResponse()
...
def accept_decline_view(request):
    ...
    if data is not "":
        ...
        if request.user.is_authenticated:
            if des == 'A':
                user_receiver.friends.add(models.UserInfo.objects.get(user=user_info))
                user_receiver.save()
                friendRequests = models.FriendRequest.objects.get(to_user=user_receiver, from_user=user_info)
                friendRequests.delete()

            elif des == 'D':
                friendRequest = models.FriendRequest.objects.get(to_user=user_receiver, from_user=user_info)
