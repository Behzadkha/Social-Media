...
function submitPost(event) {
    ...
    let sID = $("#post-text").text();
    let data = { 'postContent' : sID };
    $.post(url_path, data, successResponse);
}

function likesuccessResponse(data,status){
	if (status == 'success'){
		location.reload();
	}
	else{
...
function submitLike(event) {
    ...
    let data = { 'postID' : lID };
    $.post(url_path, data, likesuccessResponse);
}
